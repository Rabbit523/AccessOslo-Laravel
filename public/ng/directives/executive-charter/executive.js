...
var accessosloExecutivecharter = angular.module('accessosloApp.accessosloExecutivecharter', []);
var accessosloApp = angular.module('accessosloApp', ['accessosloApp.application', 'accessosloApp.accessosloExecutivecharter']);
...
'use strict';
accessosloExecutivecharter.directive('accessosloExecutivecharter', ["accessosloAPI", function (accessosloAPI) {
  return {
    templateUrl: '/ng/directives/executive-charter/view.html',
    scope: {},
    controller: function ($scope) {
            ...
            accessosloAPI.users.find(data, function (response) {          
              if (response == "error") {
                accessosloAPI.users.create(data, function (response) {
                  ...
                  $scope.$apply();
                  $(".box-description").attr('style', 'height: 1576px');
      ...
      $scope.OnPrevious = function () {
        $(".box-description").attr('style', 'height: 1356px');
        $scope.visible_step1 = true;
        $scope.visible_step2 = false;  
      };
      $("#agree").click( function(){
        if( $(this).is(':checked') ) {
          $(".additional_service").attr('style', 'display: block;');
        } else {
          $(".additional_service").attr('style', 'display: none;');
        }
      });
      $scope.OnConfirm = function () {      
        if ($('#step2_form').valid()) {                    
          var flight_data = {
            ...
            email: $scope.quoteDetails.email,
            company: $scope.quoteDetails.company,
            booking_type: "executive",
            additional_service: $scope.quoteDetails.additional_service,
            bonus: ""
          };
          if (user != null) {
            flight_data.bonus = user.bonus;
          } else {
            flight_data.bonus = 0;
          }
